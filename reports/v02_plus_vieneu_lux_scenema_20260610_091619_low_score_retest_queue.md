...
| Priority | Type | Axis | Case | Score | Evidence | Action |
|---|---|---|---|---:|---|---|
| P2 | `axis_quality_low` | RTF 生成效率 |  | 39.019 | axis quality score below 70 with sufficient coverage | inspect low-scoring cases inside this axis and retest with parameter/code fixes |

### mlx_indextts2

| Priority | Type | Axis | Case | Score | Evidence | Action |
|---|---|---|---|---:|---|---|
| P1 | `axis_quality_low` | 自然度与音频质量 |  | 58.677 | axis quality score below 70 with sufficient coverage | inspect low-scoring cases inside this axis and retest with parameter/code fixes |
...
| P1 | `performance_slow` | 音色克隆与鲁棒性 | `xling_en_to_zh` 英文参考音色说中文克隆 |  | low/slow_generation: RTF=2.1944，生成慢于实时 2 倍以上。 | 优先作为性能优化项，不一定代表音频内容异常。 |
| P1 | `performance_slow` | 音色克隆与鲁棒性 | `xling_zh_to_en` 中文参考音色说英文克隆 |  | low/slow_generation: RTF=2.0884，生成慢于实时 2 倍以上。 | 优先作为性能优化项，不一定代表音频内容异常。 |
...
| P1 | `performance_slow` | 情绪、风格与韵律控制 | `control_duration_3s` 中文三秒目标时长控制 |  | low/slow_generation: RTF=4.6316，生成慢于实时 2 倍以上。 | 优先作为性能优化项，不一定代表音频内容异常。 |
| P1 | `performance_slow` | 情绪、风格与韵律控制 | `control_emotion_angry` 中文愤怒情绪标签控制 |  | low/slow_generation: RTF=2.3578，生成慢于实时 2 倍以上。 | 优先作为性能优化项，不一定代表音频内容异常。 |
| P1 | `performance_slow` | 情绪、风格与韵律控制 | `control_emotion_happy` 中文开心情绪标签控制 |  | low/slow_generation: RTF=3.0269，生成慢于实时 2 倍以上。 | 优先作为性能优化项，不一定代表音频内容异常。 |
| P1 | `performance_slow` | 情绪、风格与韵律控制 | `control_emotion_sad` 中文悲伤情绪标签控制 |  | low/slow_generation: RTF=2.4584，生成慢于实时 2 倍以上。 | 优先作为性能优化项，不一定代表音频内容异常。 |

### mlx_ming_omni_tts

| Priority | Type | Axis | Case | Score | Evidence | Action |
|---|---|---|---|---:|---|---|
| P1 | `performance_slow` | 内容准确与可懂度 | `core_en_numbers` 英文数字、时间与价格朗读 |  | low/slow_generation: RTF=2.9019，生成慢于实时 2 倍以上。 | 优先作为性能优化项，不一定代表音频内容异常。 |
| P1 | `axis_quality_low` | 自然度与音频质量 |  | 52.25 | axis quality score below 70 with sufficient coverage | inspect low-scoring cases inside this axis and retest with parameter/code fixes |
...
| P2 | `axis_quality_low` | RTF 生成效率 |  | 16.14 | axis quality score below 70 with sufficient coverage | inspect low-scoring cases inside this axis and retest with parameter/code fixes |

### mlx_moss_tts

| Priority | Type | Axis | Case | Score | Evidence | Action |
|---|---|---|---|---:|---|---|
| P2 | `axis_quality_low` | RTF 生成效率 |  | 56.993 | axis quality score below 70 with sufficient coverage | inspect low-scoring cases inside this axis and retest with parameter/code fixes |

### mlx_omnivoice

| Priority | Type | Axis | Case | Score | Evidence | Action |
|---|---|---|---|---:|---|---|
| P0 | `case_quality_low` | 音色克隆与鲁棒性 | `xling_zh_to_en` 中文参考音色说英文克隆 | 57.144 | chosen_model=mlx_omnivoice_8bit_trim60; cer=0.125; wer=0.3333; rtf=1.2762; fail_flags=ok | inspect adapter parameters/stdout/stderr and retest this case |
| P2 | `axis_quality_low` | RTF 生成效率 |  | 45.062 | axis quality score below 70 with sufficient coverage | inspect low-scoring cases inside this axis and retest with parameter/code fixes |

### mlx_qwen3_tts

| Priority | Type | Axis | Case | Score | Evidence | Action |
|---|---|---|---|---:|---|---|
| P1 | `coverage_gap_boundary` | 多语种能力 |  | 59.151 | missing=core_vi_reading;core_hi_amount;core_ar_names | document capability boundary; do not retest unless a new supported adapter/model is added |
...
| P0 | `case_quality_low` | 音色克隆与鲁棒性 | `xling_en_to_ja` 英文参考音色说日语克隆 | 50.677 | chosen_model=mlx_scenema_audio_int8_mlx_mps_service; cer=0.3714; wer=0.3714; rtf=1.2898; fail_flags=ok | inspect adapter parameters/stdout/stderr and retest this case |
...
| P1 | `performance_slow` | 音色克隆与鲁棒性 | `xling_en_to_zh` 英文参考音色说中文克隆 |  | low/slow_generation: RTF=4.4769，生成慢于实时 2 倍以上。 | 优先作为性能优化项，不一定代表音频内容异常。 |
| P1 | `performance_slow` | 音色克隆与鲁棒性 | `xling_zh_to_en` 中文参考音色说英文克隆 |  | low/slow_generation: RTF=3.289，生成慢于实时 2 倍以上。 | 优先作为性能优化项，不一定代表音频内容异常。 |
...
| Priority | Type | Axis | Case | Score | Evidence | Action |
|---|---|---|---|---:|---|---|
| P2 | `axis_quality_low` | RTF 生成效率 |  | 41.707 | axis quality score below 70 with sufficient coverage | inspect low-scoring cases inside this axis and retest with parameter/code fixes |
